...
wii_sports_sum = calculate_total_sales(video_game_sales[0])
print(f"Sum of NA, EU, and JP sales: {wii_sports_sum:.2f} million USD.")
print("-" * 50)
...
# Test without specifying a genre (should fall back to default 'Platform')
default_filter = filter_by_genre(video_game_sales)
print(f"Default (Platform) count: {len(default_filter)} games.")

# Test specifying a different genre
racing_filter = filter_by_genre(video_game_sales, 'Racing')
print(f"Specified (Racing) count: {len(racing_filter)} games.")
print("-" * 50)
